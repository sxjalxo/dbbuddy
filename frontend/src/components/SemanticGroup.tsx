...
export function SemanticGroup({ term, columns }: { term: string; columns: SemanticColumn[] }) {
  return (
    <section className="rounded-2xl border border-border bg-card/80 p-4 shadow-[var(--shadow-soft)] animate-fade-up transition-colors duration-300 hover:border-primary/20">
      ...
      <div className="space-y-2">
        {columns.map((col, ci) => (
          <article
            key={col.table ? `${col.table}-${col.column}-${term}` : `fallback-${ci}-${col.column}`}
            style={{ animationDelay: `${ci * 50}ms` }}
            className="rounded-xl border border-border/70 bg-background/70 p-3 animate-fade-up transition-all duration-200 hover:border-primary/30 hover:bg-background"
          >
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="break-words font-mono text-sm text-foreground">
                  {col.table ? <span className="text-muted-foreground">{col.table}.</span> : null}
                  {col.column}
                </div>
